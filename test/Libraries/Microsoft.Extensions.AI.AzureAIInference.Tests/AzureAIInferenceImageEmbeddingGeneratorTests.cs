...
public class AzureAIInferenceImageEmbeddingGeneratorTests
{
    ...
    [Fact]
    public async Task GenerateAsync_DoNotOverwrite_NotNullPropertiesInRawRepresentation()
    {
        DataContent dotnetPng = new(ImageDataUri.GetImageDataUri());
        ...
        EmbeddingGenerationOptions generationOptions = new EmbeddingGenerationOptions
        {
            Dimensions = 3072,
            RawRepresentationFactory = (IEmbeddingGenerator e) =>
            {
                ImageEmbeddingsOptions azureAIOptions = new([])
                {
                    Dimensions = 1536,
                    EncodingFormat = EmbeddingEncodingFormat.Single,
                    Model = "embed-v-4-0",
        ...
        var response = await generator.GenerateAsync([dotnetPng], generationOptions);
        Assert.NotNull(response);
        Assert.Single(response);
