...
public class ApprovalGeneratingChatClient : DelegatingChatClient
{
    ...
    public ApprovalGeneratingChatClient(IChatClient innerClient, ILoggerFactory? loggerFactory = null)
        : base(innerClient)
    {
        _logger = (ILogger?)loggerFactory?.CreateLogger<ApprovalGeneratingChatClient>() ?? NullLogger.Instance;
        _ = _logger;
    }
    ...
    {
        var functionResultCallIds = new HashSet<string>(messages.SelectMany(x => x.Contents).OfType<FunctionResultContent>().Select(x => x.CallId));
        ...
        for (int i = 0; i < messageCount; i++)
        {
            ...
            var content = messages[i].Contents
                .Where(x => x is not FunctionApprovalResponseContent ||
                    (x is FunctionApprovalResponseContent approval &&
                    !functionResultCallIds.Contains(approval.FunctionCall.CallId)))
                .ToList();
